...
| セクション見出し | `## 次やる` / `## アイデア／保留` / `## 完了（アーカイブ）`（見出し文言で判定。絵文字は使わない） | セクション未認識 → タスクが未着手/保留に誤分類 |
| 親タスク | `### [ID] 件名`（角括弧必須） | `### ID 件名` は認識されない |
| 子タスク | `#### [ID] 件名（親:親ID）`（角括弧＋親表記） | 子として集約されない |
| フィールド | `- キー: 値`（太字禁止） | `- **状態**:` は認識されない |
| 完了テーブル行 | `\| YYYY-MM-DD \| ts \| 親 \| ID \| 件名 \|`（先頭セルが日付、ts=完了時刻HH:MM:SS。BT-066以前のts無し4列行も後方互換で読める） | 完了カードにならない |

> 全角コロン `：` と半角コロン `:` はどちらも許容される（正規表現 `[:：]`）。ただしセクション見出しの文言と角括弧は代替不可。
...
|-----------|--------------------|----------------------|
| h3 単発（子なし親なし） | 次やる/アイデア から削除 | **する**（完了日必須） |
...
- 完了テーブルの見出し行はどのテンプレートも `| 完了日 | ts | 親 | ID | 件名 |` に統一済み（BT-066でts列を追加した際に、長らく残っていた `inbox.backlog.md` テンプレートの "EP" という古い列名も合わせて `親` に揃えた）。パーサーは列の見出し文字列自体を見ないため実害はなかったが、統一しておくとテンプレートを見比べたときに混乱しない。
...
- **add-task**: 「次やる」セクションを探し、次の `##` セクションの直前に挿入。プロジェクトファイル名→prefix を解決して `allocateId` で採番。プロジェクト未特定なら `inbox.backlog.md`（無ければ自動生成）へ。
...
|--------------------|--------------------------------|
| セクションは「次やる」「アイデア／保留」「完了（アーカイブ）」の文言必須（絵文字は使わない） | `detectSectionType` が見出し文言でカラム判定 |
| タスクヘッダーは `[ID]` 角括弧必須 | `### \[([^\]]+)\]` 正規表現 |
| 子タスクは `（親:親ID）` 表記 | 子タイトルから親表記を除去する処理 |
| フィールドは `- キー: 値`（太字禁止） | `- (.+?)[:：] (.*)` 正規表現 |
| 状態値は既定5種のみ | `VALID_STATUSES`（config由来）で検証 |
| 完了テーブルは `\|日付\|ts\|親\|ID\|件名\|` | 完了行パース（先頭セルが日付。ts無し4列行も後方互換で読む） |
