...
| 実行中インジケータ | いまAIが着手中のタスクをグロー表示でひと目に。ステータスバーにも全ワークスペースで実行中のタスクが表示される |
...
| **GitHub Issue連携** | Issueの取り込み・起票・完了時の自動クローズまで一気通貫。詳しくは次節 |
| タスク検索 | IDやキーワードでタスクに一発ジャンプ |
| 複数選択・一括操作 | 複数タスクをまとめて編集・削除・GitHub Issue化 |
| ワークスペース連携 | カードから対応するワークスペースをVSCodeで開く（無ければその場でワークスペースを作成可能） |
| 説明欄の折りたたみ表示 | 長い説明も2行に畳んでカード一覧の見やすさを保つ。クリックで全文展開 |

より詳しい仕様は [design-docs/backlog-dashboard-spec.md](design-docs/backlog-dashboard-spec.md) を参照。

### 画面イメージ

**Epicミニボード**（親タスクを開くと、子タスクだけのミニかんばんが中に出てくる。ドラッグ&ドロップも達成感モードもここでそのまま動く）

![Epicカードを開いたときのミニボード。子タスクが進行中/未着手/完了に分かれて表示される](./images/epic-miniboard.png)

**タスク検索**（キーワードを打つだけで、全ワークスペース横断で一発ジャンプ）

![タスク検索モーダルでキーワード検索した結果一覧](./images/task-search.png)

**複数選択・一括操作**（チェックを付けて、まとめて移動・GitHub登録・削除）

![選択済みのタスクカード](./images/multiselect-card.png)
![複数選択時に下部に出てくる一括操作ツールバー](./images/multiselect-toolbar.png)

---

## GitHub Issueと二重管理にならない

チームやOSS活動ではGitHub Issueで動いているけれど、自分の頭の中の「次やること」まではIssue化していない・できていない、ということはないだろうか。

- チームはGitHub Issueで動いているのに、自分は別でTODOを管理していて、結局両方に同じことを書く**二重管理**になっている
- 「あのIssue、どのタスクの話だったっけ？」を後から探す羽目になる
- Issueをcloseし忘れる・完了を反映し忘れる、のどちらかがだいたい起きる

backlog-dashboardは、この2つの世界を自動でつなぐ。

- **既存のGitHub Issueを取り込める**: リポジトリのOpen Issue一覧を取り込みダイアログで確認し、チェックを付けるだけでバックログタスク化できる。既に取り込んだIssueには目印が付くので、二重取り込みの心配もない
- **バックログのタスクからGitHub Issueを起こせる**: 逆方向も対応。思いついたタスクをバックログに書いて、そのままIssue化できる。複数タスクをまとめて選択して一気にIssue化することもできる
- **タスクを「完了」にするだけでIssueがcloseされる**: バックログ側で完了ステータスにした瞬間、紐づいたGitHub Issueに完了コメントが自動で投稿され、Issueもクローズされる。「Issueを閉じ忘れた」を防げる
- **カードからワンクリックでIssueへ**: タスクカードにGitHubアイコンとIssue番号が表示され、詳細を見たければそのままIssueに飛べる

![GitHub Issueと紐づいたEpicカード。GitHubアイコンとIssue番号が表示されている](./images/github-card.png)

「日々のタスク感はbacklog-dashboard、Issue管理はGitHub」で使い分けても、あとから繋げる作業は一切発生しない。これができるなら、チームで動いているプロジェクトにも導入する理由になるはず。
