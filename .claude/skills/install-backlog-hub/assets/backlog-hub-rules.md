...
## md書式規約（厳守）
- セクション見出しは絵文字を使わずテキストのみ: `## 次やる` / `## アイデア／保留` / `## 完了（アーカイブ）`（BT-170で絵文字判定を廃止。見出し文言そのもので判定するため文言の変更もパーサーが壊れる）
- 親タスク: `### [ID] 件名`（角括弧必須）
- 子タスク: `#### [ID] 件名（親:親ID）`
...
## 完了・アーカイブの作法
- h3単発タスク: update-statusで完了にするとAPIが自動で次やる/アイデアから削除し、完了テーブルに1行追加する（完了日込み。BT-017で実装・検証済み）。手動でmd編集する必要はない
